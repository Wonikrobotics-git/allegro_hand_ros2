...
def generate_launch_description():
    ...
    allegro_hand_controller_fcc_spawner = Node(
        package="controller_manager",
        executable="spawner",
    ...
    analyzer_config_path = os.path.join(
        bringup_pkg_path, "config", "v4", "single_hand", "diagnostic_analyzers.yaml"
    )
    ...
    diagnostic_aggregator_node = Node(
        ...
        output="screen",
        # Instead of 'config_file', the dictionary containing the file contents is passed directly.
        parameters=[analyzer_params],
    )

    rviz_file = PathJoinSubstitution(
        [FindPackageShare("allegro_hand_bringup"), "visualize", device, "hand.rviz"]
    )
    ...
    return LaunchDescription(
        [
            ...
            allegro_hand_controller_fcc_spawner,
            allegro_hand_controller_gp_spawner,
            allegro_hand_controller_ge_spawner,
